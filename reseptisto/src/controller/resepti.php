...
function lisaaResepti($formdata) {
  ...
  if (!isset($formdata['RecipeName']) || !$formdata['RecipeName']) {
    $error['RecipeName'] = "Anna reseptin nimi.";
  } else {
    if (!preg_match("/^[- '\p{L}]+$/u", $formdata["RecipeName"])) {
      $error['RecipeName'] = "Syötä nimi ilman erikoismerkkejä.";
    }
  }
  ...
  if (!$error) {
    ...
    $idhenkilo = lisaaResepti($RecipeName);
